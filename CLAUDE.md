...
`src/state/history.js` `snapshot()` deep-clones a hand-maintained list of `state` keys. `saveStateToHistory()` is called *before* a mutation. **When you add new undoable state, you must add its key to `snapshot()` or it won't be tracked by undo/redo.** Runtime-only fields (e.g. animation `playing`/`currentTime`, the 3D mockup's `orbitProgress`) are deliberately stripped from the snapshot. (The `mockup3d` key, v21, is in the allow-list; the `tour` key, v25, is too, so hotspot/callout edits undo. `tour` also rides `PROJECT_FIELDS` in `serialize.js` so a tour's per-step hotspots persist with the project — each page is a tour step.)
