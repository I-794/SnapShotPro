...
### Open Canvas: an infinite board over `pages.js`
v32 adds a board view where every page is a card on a freeform, pan-and-zoom canvas. The load-bearing reuse: **a board card IS a page**. `pages.js` already held id-keyed pages (`{ id, payload, thumb }`) with `switchTo`/`addPage`/`deletePage`/`renderAllPages`-style offscreen render, so the board is a **DOM layer over that engine**, not a new multi-scene renderer.

- **View mode**: `state.mode === 'board'` is one early-return branch in `render()` (mirroring `'set'`), routing to `renderBoard()` in `src/features/board.js`. `renderInto` and the four composition paths are **untouched**. The board surface is a camera-transformed `.board-surface` (pan/zoom) holding absolutely-positioned `.board-card`/`.board-text` DOM nodes + an SVG `.board-connectors` overlay for arrows and group bboxes.
- **Cards vs pixels**: each card shows the cached `page.thumb` in preview; `exportBoard()` re-renders each card's page offscreen via `applyDesignToState` + `renderInto` (the same loop `pages.js` deck export uses) and composites them at their board rects. Double-click a card -> `switchTo(indexOfPage(pageId))` opens it in the single-canvas editor; "Back to board" returns with a force-refreshed thumb (`syncActivePage`).
- **Board objects** (`state.board.objects`): `card` (refs `pages[i].id`), `text`, `arrow` (`from`/`to` ids), `group` (`children` ids; `resolveBoardRef` group `moveBy` translates all children). Two **runtime-only** selection stores that are deliberately NOT in `history.snapshot()`: `canvasSelection` (scene objects, inside a card) and `boardSelection` (board objects). `src/features/board-tools.js` `resolveBoardRef` mirrors `selection.js`'s uniform handle (`box`/`moveBy`/`clone`/`remove`/`raiseToFront`/`sendToBack`) but in board px. Board spatial edits are document ops (like `addPage`), not undoable in v1; scene edits inside a card remain fully undoable.
- **Persistence is document-level**, sibling to `pages`/`active`: `pages.js` `serializeDocument()`/`applyDocument()` carry `board` (deep-cloned objects + camera); `DOC_VERSION` 13->14 with `migrateBoardV14()` (wraps pre-v32 docs in a default card grid). `SCHEMA_VERSION` stays 19 - the per-design payload is unchanged. `state.board`/`boardSelection` are NOT in `serialize.js`.
- **Mode boundaries**: `set-ui.js` `setMode` and `keyboard.js` Esc tear the board down through `window.__teardownBoardChrome`/`window.__exitBoardMode` globals (mirrors the v25 `window.__exitTourMode` guard), since the board hides `#canvas-wrapper`. `keyboard.js`'s Delete/arrow-nudge are gated to `state.mode === 'single'` so the board's own Delete (acting on `boardSelection`) doesn't double-fire.
- **Adding to the board**: new feature interactions go through `board.js` (`bindBoard()` wires camera, empty-surface marquee, text drag via delegation, Delete). Cmd-K commands: `toggleBoard`, `boardAddText`, `exportBoard`. Seed (URL->set cards) and Control (conversational agent tools) are staged follow-ons.
...
| v32 | Open Canvas (infinite board over `pages.js`) | `features/board.js` + `features/board-tools.js`; a board **card is a page** - the board is a DOM layer (camera-transformed `.board-surface` + absolutely-positioned `.board-card`/`.board-text` + an SVG `.board-connectors` overlay) laid over the existing `pages.js` engine, NOT a new multi-scene renderer. `renderInto` and the four composition paths are untouched; `render()` gets one `state.mode === 'board'` branch (mirroring `'set'`) that calls `renderBoard()`. Per-card scene pixels come from the cached `page.thumb` (preview) or offscreen `renderInto` per card (export, via `exportBoard()`). Board objects: `card` (refs `pages[i].id`), `text`, `arrow` (`from`/`to` ids), `group` (`children` ids; `moveBy` translates children). Two runtime selection stores: `canvasSelection` (scene, inside a card) and `boardSelection` (board); both non-undoable; `resolveBoardRef` mirrors `selection.js`'s handle shape. Persistence is document-level (`serializeDocument`/`applyDocument`, `DOC_VERSION` 13->14, `migrateBoardV14()`); `state.board`/`boardSelection` are NOT in `history.snapshot()` and NOT in `serialize.js`. `set-ui.js` `setMode` + `keyboard.js` Esc tear the board down via `window.__teardownBoardChrome`/`window.__exitBoardMode` (mirrors the v25 tour guard). Cmd-K commands `toggleBoard`/`boardAddText`/`exportBoard`. **v32 is the first version directed under GLM-5.2** (the model driving the brainstorm/spec/build session). Seed (URL->set) and Control (conversational agent tools) are staged as v32.1+ follow-ons, not yet built |
