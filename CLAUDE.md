...
These came out of the V24 brainstorm — "major features that don't add on to anything." **Code Snippet Studio shipped as v24** (paste code → a themed code screenshot rasterized into the canvas; see `src/features/code-snippet.js` + `src/render/code-render.js`). **Interactive Tours shipped as v25** — a new *output format*: each page becomes a clickable step with hotspots + callouts (`state.tour`, which rides the page payload), exported as one self-contained HTML player. Key files: `src/features/tours.js` (authoring mode, reuses `pages.js` for steps), `src/render/tour-overlay.js` (hotspot-authoring overlay, preview chrome outside `renderInto`), `src/features/tour-export.js` (offscreen render-all + inline vanilla player + in-app preview). MVP is linear (no branching), self-contained HTML only (no hosted URLs). **Browser extension shipped as v26** (Chrome MV3 one-click capture → studio handoff; see the "Browser extension" build note above). **V27 shipped a marketing-site redesign** — dual-mode: the dark-glass look was kept as the default and *elevated* to actually pop (purposeful glass instead of all-over, real product screenshots framed as art via the new `.frame`/`.chip` classes in `public/site.css`, solid headlines, higher contrast), plus an opt-in editorial-light "Gallery" theme behind a persistent `html[data-theme="light"]` toggle (`public/site.js` + a pre-paint `<head>` script on every page). The Impeccable skill (`.claude/skills/impeccable`, with `PRODUCT.md`/`DESIGN.md`) drove the polish. **The editor studio UI rebrand is reserved for V30** — V27 deliberately left `/editor/` on its own dark studio chrome; aligning the studio to the marketing brand is the V30 slot, not a V27 task. The rest below are unbuilt; the next flagship slot is open:
