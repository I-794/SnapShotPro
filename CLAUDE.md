...
- **Browser extension (v26)**: `chrome-extension/` is a standalone MV3 bundle (manifest + background + content scripts + popup), **NOT part of the Vite build** — load-unpacked for dev, zip for the Chrome Web Store (`chrome-extension/store/` holds listing copy + generated screenshots from `scripts/build-store-shots.mjs`). It captures a page (visible / full-page scroll-stitch / region) and hands the PNG to the studio. Cross-origin handoff: the extension can't touch the site's Cache Storage (origin-scoped), so its `content/bridge.js` relays the capture into the editor page via `window.postMessage`; `src/features/ext-receiver.js` (`bindExtReceiver()`, wired in `main.js`) validates origin+nonce on `/editor/?ext=<nonce>` and feeds it through the same `loadImage()` entry point as upload/drop/paste. The `/extension/` marketing page lives at repo `extension/index.html` (a Vite input — distinct folder from the bundle).
