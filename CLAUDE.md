...
These came out of the V24 brainstorm — "major features that don't add on to anything." **Code Snippet Studio shipped as v24** (paste code → a themed code screenshot rasterized into the canvas; see `src/features/code-snippet.js` + `src/render/code-render.js`). **Interactive Tours shipped as v25** — a new *output format*: each page becomes a clickable step with hotspots + callouts (`state.tour`, which rides the page payload), exported as one self-contained HTML player. Key files: `src/features/tours.js` (authoring mode, reuses `pages.js` for steps), `src/render/tour-overlay.js` (hotspot-authoring overlay, preview chrome outside `renderInto`), `src/features/tour-export.js` (offscreen render-all + inline vanilla player + in-app preview). MVP is linear (no branching), self-contained HTML only (no hosted URLs). **Browser extension shipped as v26** (Chrome MV3 one-click capture → studio handoff; see the "Browser extension" build note above). A **dual-mode marketing-site redesign was prototyped as an OPTION, not shipped** — dark-glass kept as the default and elevated (purposeful glass, real screenshots framed as art via new `.frame`/`.chip` classes in `public/site.css`, solid headlines, higher contrast) plus an opt-in editorial-light "Gallery" theme via `html[data-theme="light"]` (`public/site.js` toggle + a pre-paint `<head>` script). It is **parked** on branch `claude/snapshotpro-v27-brainstorm-rtnt9i` (draft PR #13 with a Vercel preview) for future consideration; it was deliberately NOT released, so the next version slot stays open. Driven by the Impeccable skill in `.claude/skills/impeccable`. (If the redesign is later adopted, aligning the `/editor/` studio chrome to it would be a natural follow-up.) The rest below are unbuilt; the next flagship slot is open:
